...
export const ProcesoEditarPage = () => {
  ...
  const onSubmit = async (data: EditarProcesoFormData) => {
    ...
    try {
      ...
      if (tieneResolucionFinal) {
        ...
        const ciclosRef = collection(db, 'procesos_sumariales', id, 'ciclos_fiscal');
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl p-8 border border-primary-100 shadow-sm space-y-6">
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              disabled={isLoading}
              className="bg-primary-200 hover:bg-primary-300 text-white font-medium py-3 px-6 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
            >
              <Save size={20} />
